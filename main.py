...
def resolve_color(candidate: Optional[str] = None) -> str:
    if candidate:
        return candidate
...
def _handle_primary_flags(args: list[str]) -> Optional[int]:
    if not args:
        return None
...
def _show_file_slice(
    ...
    *,
    ...
    if not target.exists():
        renderer.display_error(f"File not found: {target}")
        return 1

    if target.is_dir():
        renderer.display_error(f"{target} is a directory. Use --read with files only.")
        return 1
    ...
    renderer.display_info(format_file_slice_for_prompt(file_slice, rel_root=rel_root))
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    renderer = CLIRenderer(color_prefix=resolve_color())
    engine = AIEngine(renderer=renderer, config=config, default_model=config.get("model", DEFAULT_MODEL))
    ...
    if scope_candidate:
        ...
        if candidate_path.exists():
            ...
            if candidate_path.is_file():
                if not prompt_text:
                    renderer.display_info("Provide an instruction after the file path.")
                    return 1
                return engine.run_edit(scope_arg, prompt_text)
            else:
                if not prompt_text:
                    renderer.display_info("Provide a question or instruction.")
                    return 1
                return engine.run_conversation(prompt_text, scope_arg)
        else:
